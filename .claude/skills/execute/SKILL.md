...
Model policy: Codex role agents must use `gpt-5.5` with `reasoning_effort: "xhigh"` and `service_tier: "priority"`. Do not fall back to `gpt-5.4`; if `gpt-5.5` capacity is unavailable, retry later or handle the work locally. Claude role agents must use Opus 4.8.
