...
- Limit active role-agent concurrency to three by default. Count implementer, tester, PM acceptance, grooming, and on-call agents toward this cap. If fewer than three eligible independent tracks exist, run fewer; do not create duplicate agents just to fill the cap. Only exceed three when Alexey explicitly asks for a larger temporary burst.
- Use the high-capability model for role agents. In Codex, launch every role agent with `gpt-5.6`, `reasoning_effort: "high"`, and `service_tier: "priority"` unless Alexey explicitly asks for a cheaper or lower-reasoning run. Do not fall back from `gpt-5.6` to `gpt-5.5` or `gpt-5.4`; if the Codex launcher does not expose an explicit `gpt-5.6` override, omit the `model` field so the role agent inherits the parent model while still setting `reasoning_effort: "high"` and `service_tier: "priority"`. If `gpt-5.6` capacity is unavailable, keep the work local or retry `gpt-5.6`. In Claude, use Opus 4.8 for role agents unless Alexey explicitly asks for a cheaper or lower-reasoning run.
...
- Exhaustive local runs — `make test`, `make test-all`, `make coverage`, or `manage.py test` with no labels — are CI-only by default. The full Django suite is ~14,800 tests; Deploy Dev runs it sharded on every push to main (`.github/workflows/deploy-dev.yml`) and blocks the deploy on failure, so a local repeat only starves the other agents on the box. Do not run them during per-issue implementation or review unless Alexey explicitly asks. Do not treat `make coverage` as the Deploy Dev gate; Deploy Dev collects Coverage.py on the sharded `test` matrix and combines with `--fail-under=85`.
- Tester must actually run the scoped local verification — not just review code. Test report must include counts by type, the exact command(s) `scripts/affected_tests.py` emitted, and which Playwright subset (`core` vs `full`) ran.
- Testers in separate worktrees run Playwright in PARALLEL — never serialize them. As of #885 the Playwright server fixture resolves a free OS-assigned port per session (or honors `PLAYWRIGHT_DJANGO_PORT`) and each worktree uses its own `test_playwright_db.sqlite3`, so concurrent `make test-playwright[-core]` runs from different worktrees no longer collide. When two issues are in review at once, launch both testers at the same time. Two separate same-worktree local Playwright invocations are blocked by a repo-local pytest guard before migrations/server/browser startup, because they would share one sqlite DB. As of #1470 a single invocation is itself parallel: `make test-playwright[-core]` runs `pytest -n 4 --dist loadfile`, each xdist worker owning its own port and its own `test_playwright_db_gwN.sqlite3`. Tune with `PLAYWRIGHT_XDIST_WORKERS` (`0` = serial). See `_docs/testing-guidelines.md` ("Running Playwright in isolation / parallel across worktrees")
