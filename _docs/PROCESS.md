...
- Use the high-capability model for role agents. In Codex, launch every role agent with `gpt-5.5`, `reasoning_effort: "xhigh"`, and `service_tier: "priority"` unless Alexey explicitly asks for a cheaper or lower-reasoning run. Do not fall back from `gpt-5.5` to `gpt-5.4`; if `gpt-5.5` capacity is unavailable, keep the work local or retry `gpt-5.5`. In Claude, use Opus 4.8 for role agents unless Alexey explicitly asks for a cheaper or lower-reasoning run.
