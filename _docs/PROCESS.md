...
- Exhaustive local runs — `make test`, `make test-all`, `make coverage`, or `manage.py test` with no labels — are CI-only by default. The full Django suite is ~14,800 tests; CI runs it on every push to main (`.github/workflows/deploy-dev.yml`) and blocks the deploy on failure, so a local repeat only starves the other agents on the box. Do not run them during per-issue implementation or review unless Alexey explicitly asks
