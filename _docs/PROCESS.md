...
- Exhaustive local runs — `make test`, `make test-all`, `make coverage`, or `manage.py test` with no labels — are CI-only by default. The full Django suite is ~17,200 tests; Deploy Dev runs it sharded on every push to main (`.github/workflows/deploy-dev.yml`) and blocks the deploy on failure, so a local repeat only starves the other agents on the box. Do not run them during per-issue implementation or review unless Alexey explicitly asks. Do not treat `make coverage` as the Deploy Dev gate; Deploy Dev collects Coverage.py on the sharded `test` matrix and combines with `--fail-under=85`.
