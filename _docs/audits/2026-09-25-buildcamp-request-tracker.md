...
| Make Optional-content week separators portable source metadata | Implemented locally; site dev deploy and source publication pending | Screenshot 20260925-221729. Site commit `586a92d43` renders `module.syllabus_section` generically in syllabus, overview, and reader; three focused tests pass on a non-Buildcamp course. Source commit `f55fb73` sets `syllabus_section: Week N` on the first topic of each week. Localhost:8030 outputs Week 1–8 from metadata after local sync. |
...
| Keep long code blocks within the homework reader width | Implemented locally; deploy open | Screenshot 20260925-221346; AISL commit `01a700062`. Chromium layout probe at 1280/390px in dark/light confirms the 7.7kpx code line scrolls inside `<pre>` without widening the document. |
| Fix Learning in Public step layout and review the self-link | Implemented locally; deploy/source sync open | Screenshot 20260925-221420; AISL overflow commit `01a700062`, source commit `f55fb73` removes the redundant course self-link. Source push waits for generic YAML grouping renderer. |
| Shorten homework and capstone titles in navigation | Implemented locally; source publication pending | Screenshot 20260925-221630. Source commit `f55fb73` removes redundant module/week prefixes from all 10 canonical homework/capstone titles. Localhost:8030 shows “Homework: Document Processing with AI” and “Capstone: Your AI Project.” |
...
| Explain saved draft with a subtle help affordance | AISL implemented locally; shared package consumer CI pending | Screenshot 20260925-221522. AISL commit `09ab52cfd` adds a keyboard/hover accessible disclosure, verified in Chromium. Community-base branch `fix-homework-review-stale-options-help` commit `042850d` has a reusable fragment; adopt it after both consumer suites pass and release. |
