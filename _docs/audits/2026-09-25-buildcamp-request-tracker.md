...
| Remove redundant office-hours series boilerplate from every occurrence | Production updated; authenticated local render check passed | Production series 5 (`buildcamp-office-hours-cohort-4`): removed the exact paragraph from all nine editable occurrences (event IDs 57–65) and the series description for future occurrences. GET/write/GET preserved each remaining description and verified titles, dates, status, publication, access level, series linkage, recap, and recording fields. Session 1 (57) retains its recap and video; Session 2 (58) remains upcoming at its scheduled time. Authenticated local `SessionUnitPresentationTest` verifies the cleaned description is absent while title, date, subtle event link, recap, and video render. The mapped Playwright core gate is still running. |
