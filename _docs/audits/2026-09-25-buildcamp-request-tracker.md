...
| Make Optional-content week separators portable source metadata | Open | Screenshot 20260925-221729. Current `bonus_week` filter parses `Week N:` from `module.title`, while `_syllabus_module.html`, reader sidebar, and module overview branch on Buildcamp slug. Author `syllabus_section: Week N` in each Optional child `module.yaml` and render that field generically, with no title parsing or course-specific template condition. |
...
| Keep long code blocks within the homework reader width | Open; subagent working | Screenshot 20260925-221346 at `/foundation/homework/q6-structured-token-difference` shows a code fence widening the whole page and clipping the sidebar. Code should scroll inside its block; verify dark/light and narrow viewports. |
| Fix Learning in Public step layout and review the self-link | Open; layout subagent working | Screenshot 20260925-221420 shows document-wide horizontal overflow on the LIP step and a red strike through the “AI Engineering Buildcamp course” useful link. Contain layout overflow and remove or justify the redundant course self-link in authored content. |
| Shorten homework and capstone titles in navigation | Open | Screenshot 20260925-221630 strikes through “Module 1” in both titles. Use the week/module group for context and concise item names such as “Homework: Document Processing with AI” and “Capstone: Your AI Project”; review matching titles throughout Buildcamp without losing meaning. |
...
| Explain saved draft with a subtle help affordance | Open; AISL subagent working; shared port needed | Screenshot 20260925-221522 requests a small `(?)` by the draft banner, explaining autosave versus final submission. It must work on hover and keyboard focus and become a reusable community-base behavior. |
