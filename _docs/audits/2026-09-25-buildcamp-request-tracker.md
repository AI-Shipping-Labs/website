...
| Align syllabus rows and simplify right metadata | Open; Luna agent implementing | September 26 screenshots 175108, 175147: unit icons, topic chevrons, titles and right metadata should share a consistent grid. Remove the redundant right-side `Homework` label where the icon already identifies homework; keep useful counts and Optional badges. |
| Simplify syllabus due summary | Open; Luna agent implementing | September 26 screenshots 175256, 175359: say `Due <date>` instead of `Next due <date>` and remove the `· N deadlines` count. |
| Put project attempts at the first level and remove empty wrappers | Open; Luna agent investigating | September 26 screenshots 175359, 175419: show Capstone Project Attempt 1/2 as siblings of homework in the week, remove the empty `Project Attempts` folder, and add a source/sync/render integrity check that homework and project submission items cannot be nested under a topic. |
