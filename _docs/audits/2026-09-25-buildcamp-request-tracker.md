...
| Keep focused commits; push AISL; deploy dev; promote to prod | September 26 syllabus release complete; shared homework parity continues | AISL site `dc42af44c` passed Deploy Dev `36258429958`; exact image `20260926-173036-dc42af4` passed production promotion `36259987468`. |
...
| Replace broken optional-content unit links with `/c/<uuid>` share links | Source main and local preview fixed | All 24 links across seven optional pages return 302 locally. Focused course source commit `601a0b7` is on main; hierarchy draft PR AI-Shipping-Labs/ai-buildcamp-course#1 was rebased onto it. |
| Simplify Learning in Public and Review form UI; remove weighted-character copy | AISL committed; shared PR open | Course source draft PR #1 removes the phrase from all ten homework examples. AISL main has compact controls and review rows with browser save-and-submit verification. Shared behavior is in DataTalksClub/community-base#308 and draft PR #309; AISL and DTC adoption remain. |
