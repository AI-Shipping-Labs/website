...
| --- | --- | --- |
| Run AISL on localhost 8030 from the correct repository and main checkout | Running | Main serves `/courses/ai-buildcamp` with HTTP 200 at localhost:8030. The latest local source sync applied the first-level capstone rows; disabled S3 image uploads left that local sync marked partial. |
| Keep focused commits; push AISL; deploy dev; promote to prod | September 26 form UI release complete; shared homework parity continues | AISL `9499ba288` passed Deploy Dev `36268584960`; exact image `20260926-202424-9499ba2` passed production promotion `36269753534`. Production `/ping` reports the same tag and the public Buildcamp page returns HTTP 200. |
| Register and sync Buildcamp content into dev, verify rendered course pages | September 26 source synced to dev and prod; authenticated unit check open | Source `64e8f99` was forced into dev source `1dec32f0-fd53-497f-8fd6-6f0c6ebae2ba` and production source `d1a9961e-e7ba-4f52-b261-a768341a3b2e`, both with zero sync errors. Both public course pages return HTTP 200 with first-level project attempts and no old folder or deadline count. The source still lacks a webhook secret, so manual sync was used. |
...
| YAML syllabus section separation; homework/capstone as first-level rows | Deployed and verified | Source `f55fb73` supplies section headings; source `64e8f99` and site `8bac534e1` show homework and capstone attempts as first-level rows. Dev and prod source syncs and public pages were checked. |
| Align syllabus rows and simplify right metadata | Deployed and visually checked | Site `8bac534e1` aligns session, topic, homework and project icon/title grids, removes redundant right-side `Homework` labels. Desktop 1440px and narrow 390px screenshots were inspected under `.tmp/screenshots/course-syllabus-parity/`; targeted Playwright geometry check passed. |
| Simplify syllabus due summary | Deployed and verified | Site `8bac534e1` says `Due <date>` and removes the `· N deadlines` count. Public dev and prod page HTML have neither `Next due` nor deadline-count copy. |
| Put project attempts at the first level and remove empty wrappers | Deployed and verified | Source `64e8f99` moves both attempts to top-level `capstone` and deletes the empty `Project Attempts` module. Site `8bac534e1` renders both attempts as direct week rows with no extra group label. Parser rejects nested project paths; fixture checks homework and project rows at week depth 1/topic depth 0; focused Django and browser checks pass. |
| Make repository structure determine website hierarchy; remove flattening workarounds | Shared and site issues open; source migration in progress | September 26 screenshots 201425 and 201659: Week 1 homework lives under `07-homework/` and Optional Content under `08-optional-content/`, yet both are intended as first-level website items. Community-base #306 owns the generic parser, validation, mixed-sibling ordering, YAML homework convention, and projections; AISL #1830 and DTC #436 own thin adapters and removal of local hierarchy exceptions. Move source items to their intended folder level while preserving stable unit/question IDs and submissions. Structured homework fields belong in YAML, prose in Markdown. Preserve current first-level canonical URLs and `/c/<uuid>`; do not restore old nested-path redirects. |
...
| Remove redundant office-hours series boilerplate from every occurrence | Production and localhost updated; dev has no matching series; authenticated local render check passed | Production series 5 (`buildcamp-office-hours-cohort-4`): removed the exact paragraph from all nine editable occurrences (event IDs 57–65) and the series description for future occurrences. GET/write/GET preserved each remaining description and verified titles, dates, status, publication, access level, series linkage, recap, and recording fields. Localhost:8030 series 14 and events 78–86 had the same boilerplate; a single `transaction.atomic()` Django shell update changed only `description` and generated `description_html`, and read-back verified all other concrete fields stayed identical. Session 1 retains its recap and video; Session 2 remains upcoming. The dev API has no matching series and Session 1 GET returns 404, so no dev record was created. Authenticated local `SessionUnitPresentationTest` verifies the cleaned description is absent while title, date, subtle event link, recap, and video render. |
...
| Request | State | Evidence / next check |
| --- | --- | --- |
| Create community-base issue for reusable `/c/<uuid>` links in DTC; defer DTC implementation | Issue filed | DataTalksClub/community-base#310 records the shared route and access-policy requirements. |
| Replace broken optional-content unit links with `/c/<uuid>` share links | Source main, production sync and local preview fixed | All 24 links across seven optional pages return 302 locally. Focused course source commit `601a0b7` is on main and its production sync `36268113858` passed; hierarchy draft PR AI-Shipping-Labs/ai-buildcamp-course#1 was rebased onto it. |
| Simplify Learning in Public and Review form UI; remove weighted-character copy | AISL UI deployed; shared package released, adoption pending | Course source draft PR #1 removes the phrase from all ten homework examples. AISL `9499ba288` has compact controls and review rows; browser save-and-submit and desktop/mobile screenshots passed, and image `20260926-202424-9499ba2` is on dev and prod. Shared behavior is in DataTalksClub/community-base#308; PR #309 merged and v0.5.12 released after both consumer checks passed. AISL and DTC adoption remain. |
