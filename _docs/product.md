...
| Feature | URL | Description | Access | State |
|---------|-----|-------------|--------|-------|
| Past events listing | `/events?filter=past` | Canonical Events surface for every public published finished event; includes date, tags, tag filtering, pagination, gated context, and recordings highlighted with workshop-linked handoffs when available | Everyone (listing visible) | Shipped |
...
| Stripe webhook | `/api/webhooks/payments` | Receives eleven required Stripe events for immediate/delayed Checkout, subscription changes/deletion, monthly payment failure/recovery, customer email sync, and review-only refund/dispute evidence. Refunds and disputes alert operators but never change access; cancellation in Stripe remains the authoritative way to end membership. | System | Shipped |
...
Member navigates to `/events` -> sees upcoming events with spots remaining -> clicks into an event detail page -> reads description and schedule -> clicks "Register" button -> event appears in their dashboard under "Upcoming Events" -> receives notification before event -> attends via the join link near start time -> after the event ends, it becomes discoverable in the full history at `/events?filter=past`, with a recording highlighted when available. Workshop-linked recordings hand off to `/workshops/<slug>` and `/workshops/<slug>/video`; standalone recordings remain on the canonical event detail page.
...
| Past Recording | A recording available for a completed Event and highlighted within the full finished-event history at `/events?filter=past`. Workshop-linked recordings live on the Workshop landing/video pages (`/workshops/<slug>` and `/workshops/<slug>/video`); standalone past event recordings use the canonical event detail page. | Video, replay, resource |
