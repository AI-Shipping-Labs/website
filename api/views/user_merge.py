    ...
    methods={
        "POST": {
            ...
            "description": (
                ...
                "CREDENTIALS ARE DESTROYED, NOT MIGRATED: every credential "
                "counted under ``credentials`` stops working the moment a real "
                "(non-dry-run) merge commits. None of them transfer to "
                "``canonical_email``, none can be restored, and reactivating "
                "the merged-in login does not bring them back -- whoever or "
                "whatever was using one needs a replacement key issued. Member "
                "and community-base API keys are REVOKED in place, so the rows "
                "stay on the merged-in account as security history; operator "
                "tokens have no revoked state, so they are DELETED outright "
                "and leave no record behind. Run ``dry_run: true`` first and "
                "check the three counters before committing.\n\n"
