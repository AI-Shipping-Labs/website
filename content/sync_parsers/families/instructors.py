"""Instructor sync parser (family adapter over the moved dispatcher)."""

import os

from content.sync_parsers.base import FamilyParser
from content.sync_parsers.checkout_view import raise_if_checkout_error
from content.sync_parsers.common import INSTRUCTOR_ID_RE, logger
...
def _sync_instructor(source, rel_path, data, commit_sha, stats,
                     seen_ids, failed_ids):
    """Upsert one instructor YAML file (moved dispatcher body)."""
...
def _cleanup_instructors(source, stats, seen_ids, failed_ids):
    """Soft-delete instructors removed from this source (moved tail).

    We restrict the cleanup to rows that came from THIS source so that
...
class InstructorsParser(FamilyParser):
    content_type = 'instructors'
    state_name = 'instructors'

    def iter_items(self, run):
        state = self._state(run)
        for rel_path in sorted(run.classification().instructor_files):
            filepath = os.path.join(run.repo_dir, rel_path)
            data = None
            try:
                data = _parse_yaml_file(filepath)
                # Required: id, name. ``_validate_frontmatter`` raises a
                # clear ValueError that the per-file error capture records
                ...
                try:
                    ...
                    failed_id = ''
                state.failed.add(failed_id)
                state.record_error({'file': rel_path, 'error': str(e)})
                continue
            identity = None
            if isinstance(data, dict):
                identity = str(data.get('id', '')).strip() or None
            yield rel_path, {
                'rel_path': rel_path,
                'data': data,
                'identity': identity,
    ...
    def process(self, run, payload):
        ...
        try:
            _sync_instructor(
                ...
                run.commit_sha, stats, state.seen, state.failed,
            )
        except Exception as e:  # noqa: BLE001 - bounded per-file capture
            raise_if_checkout_error(e)
            failed_id = payload.get('identity') or ''
            state.failed.add(failed_id)
            stats['errors'].append({'file': payload['rel_path'], 'error': str(e)})
