...
class HomepageMobileLayoutTest(TestCase):
    ...
    def test_view_past_event_recordings_link_has_tap_target(self):
        """Past event recordings link should have py-2 and px-3 for adequate mobile tap target."""
        content = self._get_homepage_content()
        match = re.search(r'<a[^>]*href="/events\?filter=past"[^>]*>', content)
        self.assertIsNotNone(match, "Past event recordings link not found")
