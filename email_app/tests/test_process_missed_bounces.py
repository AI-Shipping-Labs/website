...
- The ``bounce_state`` conditional path is exercised by stubbing the
  attribute on a fresh model class -- the User model in the live
  schema does not (yet) carry ``bounce_state``, so the default code
  path covers the today-world.
"""
...
class ProcessMissedBouncesDryRunTest(TestCase):
    def test_dry_run_writes_no_user_changes_and_no_ses_event(self):
        ...
        user.tags = []
        user.save(update_fields=["unsubscribed", "tags"])
...
class ProcessMissedBouncesBounceStateBranchTest(TestCase):
    """The ``hasattr(user, "bounce_state")`` branch fires when #766 lands.

    The live User model does not (yet) define ``bounce_state``. We
    simulate the post-#766 world by stubbing the User instance the
    command fetches from the DB so it carries the new attributes, and
    by patching ``User.save`` so the test does not need a schema
    migration for the new columns.
    """

    def test_bounce_state_branch_writes_structured_fields(self):
        user = User.objects.create_user(email="taylor@example.com")
        user.unsubscribed = False
        user.tags = []
        user.save(update_fields=["unsubscribed", "tags"])
        ...
        def fake_save(self_user, **kwargs):
            captured["update_fields"] = list(
                kwargs.get("update_fields") or []
            )
            captured["bounce_state"] = getattr(
                self_user, "bounce_state", None,
            )
            captured["bounce_recorded_at"] = getattr(
                self_user, "bounce_recorded_at", None,
            )
            captured["last_bounce_diagnostic"] = getattr(
                self_user, "last_bounce_diagnostic", None,
            )
            captured["unsubscribed"] = self_user.unsubscribed

        # Return a user object that LOOKS post-#766: the new attributes
        # are pre-seeded on the instance so ``hasattr`` returns True.
        stubbed_user = User.objects.get(pk=user.pk)
        stubbed_user.bounce_state = "active"
        stubbed_user.bounce_recorded_at = None
        stubbed_user.last_bounce_diagnostic = ""

        # The command's ``User.objects.filter(email__iexact=...).first()``
        # path returns ``stubbed_user``. The idempotency check (a separate
        # SesEvent query) is unaffected by this patch.
        with mock.patch(
            "email_app.management.commands.process_missed_bounces."
            "User.objects.filter",
        ) as filter_mock:
            qs = mock.MagicMock()
            qs.first.return_value = stubbed_user
            filter_mock.return_value = qs

            with mock.patch.object(
                User, "save", autospec=True, side_effect=fake_save,
            ):
                _call("--email", "taylor@example.com")

        # The structured path ran -- all four fields appear in the save's
        # update_fields list, the diagnostic is the backfill marker, and
        # unsubscribed is flipped.
        self.assertIn("bounce_state", captured["update_fields"])
        self.assertIn("bounce_recorded_at", captured["update_fields"])
        self.assertIn("last_bounce_diagnostic", captured["update_fields"])
        self.assertIn("unsubscribed", captured["update_fields"])
        self.assertTrue(captured["unsubscribed"])
        self.assertEqual(
            captured["last_bounce_diagnostic"],
            "backfilled via process_missed_bounces",
        )
        self.assertIsNotNone(captured["bounce_recorded_at"])
