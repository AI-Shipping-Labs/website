...
_DONOR_KEY_EXTRA_METADATA = {"STRIPE_MONTHLY_PAYMENT_GRACE_MODE": {"allowed_values": ["observe", "enforce"]}}
...
LOGFIRE_ENABLED = declare(
    ...
    description="Explicit on switch for Pydantic Logfire. Default off everywhere; must be true (plus a token, plus not running tests) before Logfire initializes. Keeps local/dev/eval runs silent unless an operator opts in.",
    value_type="bool",
    default="false",
    secret=False,
...
LOGFIRE_TOKEN = declare(
    ...
    group="observability",
    label="Logfire Token",
    description="Logfire write token. Get it from the Logfire project settings. When blank, Logfire is fully off. Masked in Studio.",
    value_type="str",
    default="",
    secret=True,
...
LOGFIRE_ENVIRONMENT = declare(
    ...
    description='Logfire environment tag passed to logfire.configure(environment=...), so prod traces are separable from any opt-in dev run. Defaults to "production".',
    value_type="str",
    default="production",
    secret=False,
    optional=True,
