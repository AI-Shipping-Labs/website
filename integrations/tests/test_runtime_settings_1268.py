    ...
    def test_registry_metadata_and_documentation_anchors(self):
        ...
        for group_name, (key, filename, multiline, default) in expectations.items():
            with self.subTest(key=key):
                entries = {item["key"]: item for item in get_group_by_name(group_name)["keys"]}
                entry = entries[key]
                ...
                docs = (Path(settings.BASE_DIR) / "_docs" / "integrations" / filename).read_text(encoding="utf-8")
                self.assertIn(f"## {key}\n", docs)
...
class RuntimeSettingsStudioTest(TestCase):
    ...
    def test_fields_render_with_controls_docs_and_source_badges(self):
        ...
        stripe_field = next(field for field in stripe_group["fields"] if field["key"] == "STRIPE_PAYMENT_LINKS")
...
class RuntimeSettingsApiTest(TestCase):
    ...
    def setUpTestData(cls):
        cls.staff = User.objects.create_user(
            email="runtime-settings-api@test.com",
            is_staff=True,
        )
    ...
    def test_get_lists_metadata_without_values(self):
        set_package_override("STRIPE_PAYMENT_LINKS", "do-not-leak", actor_ref="test")
        response = self.client.get("/api/integrations/settings", **self.auth)
        entries = {item["key"]: item for item in response.json()["settings"]}
    ...
    def test_post_sets_and_clears_all_three_without_echo(self):
        ...
        response = self.client.post(
            "/api/integrations/settings",
            data=json.dumps({"updates": [{"key": key, "value": ""} for key in KEYS.values()]}),
            content_type="application/json",
            **self.auth,
        )
        self.assertEqual(response.status_code, 200)
