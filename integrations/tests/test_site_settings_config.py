...
from integrations.config import clear_config_cache, get_config, set_package_override, site_base_url
...
class SiteBaseUrlConfigTest(TestCase):
    ...
    def test_db_value_overrides_env_for_site_base_url(self):
        set_package_override("SITE_BASE_URL", "https://override.example.com", actor_ref="test")
        clear_config_cache()
    ...
    def test_cache_cleared_after_db_write(self):
        ...
        set_package_override("SITE_BASE_URL_ALIASES", "https://configured.example.com", actor_ref="test")
        clear_config_cache()
...
class WorkerConfigCacheTest(TestCase):
    ...
    def test_worker_process_bypasses_in_process_config_cache(self):
        set_package_override("SITE_BASE_URL_ALIASES", "https://old.example.com", actor_ref="test")
        clear_config_cache()
        self.assertEqual(
...
class SiteSettingsSaveViewTest(TestCase):
    ...
    def test_save_creates_site_keys(self):
        ...
        self.assertEqual(
            get_config("EVENT_DISPLAY_TIMEZONE"),
            "Europe/Berlin",
        )
        # The package store has no group column: the group lives on the
        # declaration and every override is visible in the package audit
        # trail.
        self.assertTrue(
            Setting.objects.filter(key="SITE_BASE_URL").exists(),
        )
...
class CrossProcessCacheInvalidationTest(TestCase):
    ...
    def test_save_in_one_process_visible_in_another(self):
        ...
        with patch.object(shared_cache_module, "monotonic_time", fake_now):
            ...
            set_package_override("SITE_BASE_URL", "https://prod.example.com", actor_ref="test")
    ...
    def test_unchanged_stamp_does_not_repopulate(self):
        ...
        set_package_override("SITE_BASE_URL", "https://first.example.com", actor_ref="test")
        clear_config_cache()
        first = get_config("SITE_BASE_URL", "")
        self.assertEqual(first, "https://first.example.com")

        # Mutate the DB DIRECTLY without publishing a stamp. No stamp
        # change → the in-process cache must still serve the old value
        # (this is the hot-path guarantee: AC #3 forbids a config query
        # on every get_config when nothing has changed).
        Setting.objects.filter(key="SITE_BASE_URL").update(
    ...
    def test_site_base_url_helper_picks_up_cross_process_save(self):
        ...
        with patch.object(shared_cache_module, "monotonic_time", fake_now):
            reset_local_shared_cache_memo()
            set_package_override("SITE_BASE_URL", "https://aishippinglabs.com", actor_ref="test")
            clear_config_cache()
