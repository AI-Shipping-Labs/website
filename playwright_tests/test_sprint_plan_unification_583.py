...
def _seed_workspace(
    ...
    from plans.models import Checkpoint, Deliverable, NextStep, Plan, Resource, Sprint, SprintEnrollment, Week
...
class TestUnifiedWorkspace583:
    ...
    def test_owner_can_inline_edit_without_separate_edit_page(
        ...
        try:
            ...
            assert page.locator(
                '[data-testid="my-plan-edit-cta"]').count() == 0
    ...
    def test_single_column_layout_and_section_order(
        ...
        try:
            ...
            deliverables = page.locator(
                '[data-testid="plan-deliverables"]').bounding_box()
            next_steps = page.locator(
                '[data-testid="plan-next-steps"]').bounding_box()
            ...
            sections = {
                'weeks': page.locator(
                    '[data-testid="plan-weeks"]').bounding_box(),
                'resources': page.locator(
                    '[data-testid="plan-resources"]').bounding_box(),
                'action_items': page.locator(
                    '[data-testid="plan-action-items"]').bounding_box(),
                'summary': page.locator(
                    '[data-testid="plan-summary"]').bounding_box(),
            }
    ...
    def test_timeline_section_has_no_filler_copy(
        ...
        try:
            ...
            assert page.get_by_text(
                'Checkpoints are the primary flow for this sprint plan.').count() == 0
        finally:
            context.close()
    ...
    def test_teammate_sees_shared_plan_without_edit_affordances(
        ...
        try:
            ...
            expect(
                page.locator('[data-testid="plan-checkpoint"]').first).to_be_visible()
            # No edit affordances.
            assert page.locator(
                '[data-testid="plan-row-done-toggle"]').count() == 0
            assert page.locator(
                '[data-testid="plan-item-edit"]').count() == 0
            assert page.locator(
                '[data-testid="plan-visibility-toggle"]').count() == 0
            assert page.locator(
                '[data-testid="my-plan-edit-cta"]').count() == 0
            # Comments composer is present (teammates can comment on
            # shared plans).
            expect(
                page.locator('[data-testid="plan-comments-section"]')).to_be_visible()
        finally:
            context.close()
...
class TestLegacyEditUrlRedirect583:
    ...
    def test_legacy_edit_url_lands_on_unified_workspace(
        ...
        try:
            ...
            expect(
                page.locator('[data-testid="plan-visibility-toggle"]')).to_be_visible()
        finally:
            context.close()
...
class TestVisibilityToggle583:
    ...
    def test_toggle_flips_private_to_shared_in_one_click(
        ...
        try:
            ...
            assert page.locator(
                '[data-testid="visibility-save"]').count() == 0
            ...
            expect(
                page.locator('[data-testid="plan-visibility-toggle"]')).to_have_attribute('aria-checked', 'true')
        finally:
            context.close()
    ...
    def test_cohort_board_reflects_toggle_state_for_teammate(
        ...
        try:
            ...
            assert tpage.locator(
                f"a[href='/sprints/{data['sprint_slug']}/plans/{data['plan_id']}']").count() >= 1
            ...
            assert tpage.locator(
                f"a[href='/sprints/{data['sprint_slug']}/plans/{data['plan_id']}']").count() == 0
        finally:
            owner_ctx.close()
            teammate_ctx.close()
    ...
    def test_failed_save_reverts_toggle_and_shows_inline_error(
        ...
        try:
            ...
            page.reload(wait_until='domcontentloaded')
            expect(
                page.locator('[data-testid="plan-visibility-toggle"]')).to_have_attribute('aria-checked', 'false')
        finally:
            context.close()
    ...
    def test_week_note_post_shows_messages_region_only_once(
        ...
        try:
            ...
            add_form = page.locator(
                '[data-testid="plan-week-note-add-form"]').first
            add_form.locator(
                '[data-testid="plan-week-note-add-textarea"]').fill('Tried out the new agent.')
            add_form.locator(
                '[data-testid="plan-week-note-add-submit"]').click()

            # After the redirect we should see EXACTLY one messages
            # region and zero of the legacy in-page plan-messages block.
            page.wait_for_load_state('domcontentloaded')
            assert page.locator(
                '[data-testid="messages-region"]').count() == 1
            assert page.locator(
                '[data-testid="plan-messages"]').count() == 0
        finally:
            context.close()
