...
class TestMarkCompletedPersistsAcrossReload:
    ...
    def test_button_state_persists_on_reload(
        ...
        page = ctx.new_page()
        page.goto(
            f'{django_server}/workshops/{workshop.url_key}/intro',
...
class TestWorkshopShowsInContinueLearning:
    ...
    def test_workshop_card_appears_with_progress(
        ...
        assert cta.get_attribute('href') == (
            f'/workshops/{workshop.url_key}/setup'
        )
...
class TestFinishingLastPageRemovesFromContinueLearning:
    def test_workshop_drops_off_after_final_page(
        ...
        page.goto(
            f'{django_server}/workshops/{workshop.url_key}/deploy',
            wait_until='domcontentloaded',
        )
        ...
        # With no active learning, the optional commitment zone is omitted;
        # the actionable home feed remains available.
        assert page.locator(
            '[data-testid="dashboard-continue-learning-section"]'
        ).count() == 0
        assert page.locator('[data-testid="dashboard-home-feed"]').is_visible()
