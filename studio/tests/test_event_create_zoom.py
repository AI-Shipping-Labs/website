...
class EventCreateZoomTemplateTest(TestCase):
    ...
    def test_template_js_sets_red_class_on_error(self):
        """The JS code sets red text class on error responses."""
        event = Event.objects.create(
            title='Red Error', slug='red-error',
            event_type='live', platform='zoom',
            start_datetime=timezone.now(),
        )
        response = self.client.get(f'/studio/events/{event.pk}/edit')
    ...
    def test_template_js_disables_button_during_request(self):
        """The JS code disables the button and changes text to Creating... during request."""
        event = Event.objects.create(
            title='Disable Test', slug='disable-test',
            event_type='live', platform='zoom',
            start_datetime=timezone.now(),
        )
        response = self.client.get(f'/studio/events/{event.pk}/edit')
    ...
    def test_template_js_reloads_on_success(self):
        """The JS code reloads the page on successful Zoom creation."""
        event = Event.objects.create(
            title='Reload Test', slug='reload-test',
            event_type='live', platform='zoom',
            start_datetime=timezone.now(),
        )
        response = self.client.get(f'/studio/events/{event.pk}/edit')
    ...
    def test_template_js_sends_csrf_token(self):
        ...
        event = Event.objects.create(
            title='CSRF Test', slug='csrf-test',
            event_type='live', platform='zoom',
            start_datetime=timezone.now(),
        )
        response = self.client.get(f'/studio/events/{event.pk}/edit')
