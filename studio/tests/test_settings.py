...
class BooleanSettingRoundTripTest(TestCase):
    ...
    def test_checkbox_unchecked_stores_false_not_empty(self):
        ...
        set_package_override("SLACK_ENABLED", "true", actor_ref="test")
        self.client.post(
            "/studio/settings/slack/save/",
            {
...
class TypedSettingRegistryTest(TestCase):
    def test_representative_scalar_types_are_explicit(self):
        definitions = {key_def["key"]: key_def for group in INTEGRATION_GROUPS for key_def in group["keys"]}
        expected = {
            "SITE_BASE_URL": "url",
...
class AtomicTypedSettingSaveTest(TestCase):
    ...
    def _seed(self, key, value, group):
        # Group membership now lives on the package declaration.
        return set_package_override(key, value, actor_ref="test")

    def _group_keys(self, group):
        for group_def in INTEGRATION_GROUPS:
            if group_def["name"] == group:
                return [key_def["key"] for key_def in group_def["keys"]]
        return []

    def _assert_unchanged_after_invalid(self, group, payload, key, type_label, section):
        keys = self._group_keys(group)
        before = {k: get_config(k) for k in keys}
        response = self.client.post(
            f"/studio/settings/{group}/save/",
            payload,
            follow=True,
        )
    ...
    def test_valid_values_count_clears_and_boolean_override(self, clear_cache):
        ...
        self.assertFalse(Setting.objects.filter(key="SLACK_ENABLED").exists())
        self.assertEqual(
