...
class StudioSyncDashboardTest(TestCase):
    ...
    def test_dashboard_renders_one_button_per_repo(self):
        ...
        self.assertEqual(
            html.count('action="/studio/sync/AI-Shipping-Labs/content/trigger-repo/"'),
            2,
        )
    ...
    def test_dashboard_shows_tiers_synced(self):
        ...
        SyncLog.objects.create(
            source=source,
            status="success",
            warnings=[{"asl_compat": {"tiers_synced": True, "tiers_count": 3}}],
            finished_at=timezone.now(),
        )
        response = self.client.get("/studio/sync/")
    ...
    def test_dashboard_does_not_leak_other_repos_logs_via_batch_id(self):
        ...
        SyncLog.objects.create(
            ...
            items_updated=10,
            items_detail=[
                {"title": "C", "slug": "c", "action": "updated", "content_type": "course"},
            ],
            finished_at=timezone.now(),
        )
        ...
        SyncLog.objects.create(
            ...
            items_updated=10,
            items_detail=[
                {"title": "P", "slug": "p", "action": "updated", "content_type": "project"},
            ],
            finished_at=timezone.now(),
        )
...
class StudioSyncDashboardCourseBreakdownTest(TestCase):
    ...
    def test_course_breakdown_present_in_context(self):
        ...
        self.assertEqual([b["level"] for b in breakdown], ["course", "module", "unit"])
    ...
    def test_course_breakdown_counts_match_items(self):
        ...
        breakdown = {
            b["level"]: b
            for b in repos["AI-Shipping-Labs/python-course"]["last_batch"]["per_type"][0]["course_breakdown"]
        }
...
class StudioSyncDashboardCourseTreeTest(TestCase):
    ...
    def test_tree_nests_modules_under_their_course(self):
        ...
        response = self.client.get("/studio/sync/")
        tree = response.context["repos"][0]["last_batch"]["per_type"][0]["course_tree"]
    ...
    def test_article_only_batch_keeps_flat_layout(self):
        ...
        SyncLog.objects.create(
            ...
            items_created=2,
            items_detail=[
                {"title": "Hello", "slug": "hello", "action": "created", "content_type": "article"},
                {"title": "World", "slug": "world", "action": "created", "content_type": "article"},
            ],
            finished_at=timezone.now(),
        )
        response = self.client.get("/studio/sync/")
...
class StudioSyncDashboardLiveIndicatorTest(TestCase):
    ...
    def test_dashboard_wraps_repos_in_polling_wrapper(self):
        ...
        self.assertContains(
            response,
            'data-fragment-url="/studio/sync/?fragment=status"',
        )
...
class StudioSyncHistoryTest(TestCase):
    ...
    def test_history_shows_tiers_synced(self):
        SyncLog.objects.create(
            source=self.source,
            status="success",
            warnings=[{"asl_compat": {"tiers_synced": True, "tiers_count": 4}}],
            finished_at=timezone.now(),
        )
        response = self.client.get("/studio/sync/history/")
...
class StudioSyncTriggerTest(TestCase):
    ...
    def test_trigger_redirects_to_sync_dashboard(self):
    ...
    def test_trigger_calls_sync(self, mock_enqueue):
        self.client.post(f"/studio/sync/{self.source.pk}/trigger/")
        mock_enqueue.assert_called_once_with(self.source, force=False)
    ...
    def test_trigger_handles_sync_error(self, mock_queue):
    ...
    def test_trigger_handles_enqueue_failure(self):
        # A2.3: there is no inline fallback; the enqueue service reports
        # the failure and the trigger surfaces it without lying about a
        # queued state.
        with (
            patch(
                "integrations.services.content_sync_queue.package_queue_source_sync",
    ...
    @patch("studio.views.sync.enqueue_content_sync")
    def test_trigger_only_syncs_targeted_source(self, mock_enqueue):
...
class StudioSyncRepoTriggerTest(TestCase):
    ...
    def test_repo_trigger_enqueues_one_task(self, mock_enqueue):
        ...
        response = self.client.post("/studio/sync/AI-Shipping-Labs/content/trigger-repo/")
        self.assertEqual(response.status_code, 302)
        self.assertEqual(mock_enqueue.call_count, 1)
        self.assertEqual(
            [s.pk for s in mock_enqueue.call_args.args[0]],
            [source.pk],
        )
    ...
    @patch("studio.views.sync.enqueue_content_syncs")
    def test_repo_trigger_creates_batch_id(self, mock_enqueue):
        """The trigger creates a batch_id for the enqueued tasks."""
        ContentSource.objects.create(
            slug="content",
            repo_name="AI-Shipping-Labs/content",
        )
    ...
    @patch("studio.views.sync.enqueue_content_syncs")
    def test_repo_trigger_doesnt_touch_other_repos(self, mock_enqueue):
        ...
        self.client.post("/studio/sync/AI-Shipping-Labs/content/trigger-repo/")
        synced_pks = {s.pk for s in mock_enqueue.call_args.args[0]}
        self.assertEqual(synced_pks, {target.pk})
        self.assertNotIn(other.pk, synced_pks)

    def test_repo_trigger_redirects_to_sync_dashboard(self):
    ...
    def test_repo_trigger_flash_names_the_repo(self):
        ContentSource.objects.create(
            slug="content",
            repo_name="AI-Shipping-Labs/content",
        )
    ...
    def test_repo_trigger_requires_post(self):
        ...
        response = self.client.get("/studio/sync/AI-Shipping-Labs/content/trigger-repo/")
        self.assertEqual(response.status_code, 405)
    ...
    def test_repo_trigger_requires_staff(self):
        ...
        client = Client()
        response = client.post("/studio/sync/AI-Shipping-Labs/content/trigger-repo/")
        self.assertEqual(response.status_code, 302)
...
class StudioSyncAllTest(TestCase):
    ...
    def test_sync_all_redirects_to_sync_dashboard(self):
    ...
    @patch("studio.views.sync.enqueue_content_syncs")
    def test_sync_all_triggers_all_sources(self, mock_enqueue):
        ContentSource.objects.create(
            slug="blog",
            repo_name="AI-Shipping-Labs/blog",
        )
    ...
    @patch("studio.views.sync.enqueue_content_syncs")
    def test_sync_all_passes_batch_id(self, mock_enqueue):
        """Sync All passes a shared batch_id to every enqueued source."""
        ContentSource.objects.create(
            slug="blog",
            repo_name="AI-Shipping-Labs/blog",
        )
...
class StudioSyncStatusTest(TestCase):
    ...
    def test_status_returns_empty_for_never_synced(self):
        source = ContentSource.objects.create(
            slug="content",
            repo_name="AI-Shipping-Labs/content",
        )
        ...
        data = json.loads(response.content)
        self.assertEqual(data["last_sync_status"], "")
...
class SyncLogModelTest(TestCase):
    ...
    def test_tiers_synced_field(self):
        # A2.3: the package SyncLog has no tier columns; the counters ride
        # in the namespaced warnings entry the wrapper writes.
        log = SyncLog.objects.create(
            source=self.source,
            status="success",
            warnings=[{"asl_compat": {"tiers_synced": True, "tiers_count": 3}}],
        ...
        log.refresh_from_db()
        compat = next(
            (e["asl_compat"] for e in log.warnings if "asl_compat" in e),
            {},
        )
        self.assertTrue(compat.get("tiers_synced"))
        self.assertEqual(compat.get("tiers_count"), 3)
...
class StudioSyncHistoryItemLinksTest(TestCase):
    ...
    def test_existing_link_types_still_render(self):
        ...
        SyncLog.objects.create(
            ...
            items_created=5,
            items_detail=[
                {"title": "A", "slug": "a", "action": "created", "content_type": "article"},
                {"title": "C", "slug": "c", "action": "created", "content_type": "course"},
                {"title": "P", "slug": "p", "action": "created", "content_type": "project"},
                {"title": "E", "slug": "e", "id": 42, "action": "created", "content_type": "event"},
                {"title": "IQ", "slug": "iq", "action": "created", "content_type": "interview_question"},
            ],
            finished_at=timezone.now(),
        )
        response = self.client.get("/studio/sync/history/")
