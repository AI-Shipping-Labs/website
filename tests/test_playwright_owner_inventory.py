...
class InventoryRatchetTests(SimpleTestCase):
    ...
    def manifest(self, *, declared=None, non_browser=None):
        ...
        } if non_browser is None else non_browser
        return {
            "schema_version": 1,
    ...
    def validate(self, collected, manifest=None, declared_ceiling=None, non_browser_ceiling=None):
...
class CurrentRepositoryInventoryTests(SimpleTestCase):
    ...
    def test_reviewed_recap_owner_stays_in_browser_partition(self):
        ...
        self.assertEqual(len(manifest["LEGACY_DECLARED_BROWSER"]), 2258)
    ...
    def test_current_collection_exactly_matches_live_partition_without_runtime_startup(self):
        ...
        databases_before = {
            path: (path.stat().st_size, path.stat().st_mtime_ns) for path in database_paths
        }

        try:
            owners, item_count = collect_owner_ids(
                extra_env={"PLAYWRIGHT_DJANGO_PORT": "65534"}
            )
        ...
        self.assertEqual(item_count, 2559)
        self.assertEqual(len(owners), 2339)
        self.assertEqual(errors, [])
        if guard is None:
