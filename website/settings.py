...
INSTALLED_APPS = [
    ...
    'django_q',
    # Community-base package (plan issues A0.3 and A0.2). Declarations
    # only: the kernel app has no models, and neither the config app nor
    # the api app contacts Relay; no jobs or mail backend is activated
...
TEMPLATES = [
    {
        ...
        'OPTIONS': {
            'builtins': [
                'accounts.templatetags.date_formatting',
            ],
            'context_processors': [
...
#
# - journal_mode=WAL outside tests: writers don't block readers, and one
#   writer can run while others queue on the busy_timeout instead of failing
#   immediately. Tests use DELETE so Django's SQLite clone step copies a
#   complete main database file.
...
if DATABASES['default']['ENGINE'] == 'django.db.backends.sqlite3':
    if TESTING:
        # Django clones file-backed SQLite test databases by copying only the
        # main .sqlite3 file. WAL can leave migrated schema in the -wal sidecar
        # at clone time, producing incomplete --parallel --keepdb clones.
    ...
    if os.environ.get('DJANGO_TEST_DB_NAME'):
        DATABASES['default'].setdefault('TEST', {})['NAME'] = os.environ['DJANGO_TEST_DB_NAME']
...
# Community-base package configuration (plan issues A0.3 and A0.2). The
# access policy wires the site's tier semantics into the kernel protocol;
# the backend keys name the sites' existing owners and activate nothing.
# Only keys with site-side implementations are configured.
COMMUNITY_BASE = {
    'SITE_KEY': 'aisl',
